...
await capture("01-studio-desktop", "/", { width: 1440, height: 900 });
await capture("02-login-desktop", "/login/", { width: 1440, height: 900 });
await capture("03-course-desktop", "/course/", { width: 1440, height: 900 });
await capture("04-phase-desktop", "/phase/03/", { width: 1440, height: 900 });
await capture("05-course-progress", "/course/", { width: 1440, height: 900 }, (page) => page.locator(".progress-board").scrollIntoViewIfNeeded());
await capture("06-chapter-01", "/chapter/01/", { width: 1440, height: 900 });
await capture("07-chapter-13", "/chapter/13/", { width: 1440, height: 900 });
await capture("08-chapter-24", "/chapter/24/", { width: 1440, height: 900 });
await capture("09-chapter-drawer", "/chapter/13/", { width: 1440, height: 900 }, (page) => page.locator(".reader-topbar [data-sidebar-open]").click());
await capture("10-chapter-evidence", "/chapter/13/", { width: 1440, height: 900 }, (page) => page.locator(".evidence-figure").first().scrollIntoViewIfNeeded());
await capture("11-course-1280", "/course/", { width: 1280, height: 800 });
await capture("12-chapter-1024", "/chapter/13/", { width: 1024, height: 768 });
await capture("13-course-tablet", "/course/", { width: 768, height: 1024 });
await capture("14-course-mobile", "/course/", { width: 390, height: 844 });
await capture("15-studio-mobile", "/", { width: 390, height: 844 });
await capture("16-chapter-mobile", "/chapter/13/", { width: 390, height: 844 });
await capture("17-chapter-compact", "/chapter/24/", { width: 320, height: 720 });
...
if (errors.length) {
  ...
  console.log(JSON.stringify({ screenshots: 17, errors: 0 }));
}
